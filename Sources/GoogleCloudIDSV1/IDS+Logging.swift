...
extension Clients {
  final class IDSLogging: IDSStub {
    let inner: any IDSStub
    let logger: Logger

    public init(_ inner: any IDSStub, logger: Logger) {
      var logger = logger
      logger[metadataKey: "gcp.artifact.id"] = "google-cloud-ids-v1"
      logger[metadataKey: "gcp.client.service"] = "ids"
      logger[metadataKey: "gcp.experimental.swift.client"] = "IDS"
      self.inner = inner
      self.logger = logger
    }
